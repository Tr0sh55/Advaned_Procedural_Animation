...
public class SpiderController : MonoBehaviour
{
    ...
    [SerializeField] private float bodyYOffset = 0.5f;
    ...
    void FixedUpdate()
    {
        ...
        if (valueX != 0 || valueY != 0)
        {
            ...
            Vector3[] pos = GetClosestPoint(transform.position, transform.forward, upward, 0.5f, 0.1f, 30, -30, raysNb);
            
            transform.position = Vector3.Lerp(lastPosition, GetPositionAboveGround(), 1f / (1f + smoothness));
    ...
    private Vector3 GetPositionAboveGround()
    {
        RaycastHit hit;
        if (Physics.Raycast(transform.position, -upward, out hit, 1f))
        {
            return hit.point + upward * bodyYOffset;
        }
    ...
    private void OnDrawGizmos()
    {
        if (pn != null)
        {
            ...
            Gizmos.DrawSphere(pn[0], 0.1f);
            Gizmos.color = Color.green;
            Gizmos.DrawRay(pn[0], pn[1]);
        }
        
        for(int i = 0; i < raysNb; ++i)
        {
            Gizmos.color = Color.red;
